with category_gmv_aov_cte as (
    select 
        ...
        sum(fct_order_items.item_total) / count(distinct fct_order_items.order_id) as daily_aov

    from {{ref ('fct_order_items')}} as fct_order_items
        left join {{ref ('fct_orders')}} fct_orders on fct_order_items.order_id = fct_orders.order_id
...
select 
    ...
    order_count,
    daily_aov,
    daily_gmv / sum(daily_gmv) over (partition by order_date) as category_gmv_share 
